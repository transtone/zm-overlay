...
COMMON_TARGETS="i386 x86_64 alpha arm cris m68k microblaze mips mipsel ppc ppc64 sh4 sh4eb sparc sparc64"
...
RDEPEND="
	...
	amd64? ( spice? ( >=app-emulation/spice-0.9.0
...
pkg_setup() {
	...
	if use spice && ! use amd64 ; then
		eerror "Building Spice server on 32-bit machine is not supported"
		eerror "because Spice server is dependent on 64-bit atomic operations."
		eerror "However, you can build Spice client for a 32-bit machine."
		die
	fi
