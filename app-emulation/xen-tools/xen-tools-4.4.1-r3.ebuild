...
inherit bash-completion-r1 eutils flag-o-matic multilib systemd python-single-r1 toolchain-funcs udev ${live_eclass}
...
IUSE="api custom-cflags debug doc flask hvm qemu ocaml ovmf +pam python pygrub systemd screen static-libs system-qemu system-seabios"
...
src_install() {
	...
	if use systemd; then
		...
		systemd_dounit "${FILESDIR}"/{proc-xen,var-lib-xenstored}.mount
		systemd_dotmpfilesd "${FILESDIR}"/xen.tmpfiles.conf
	fi
