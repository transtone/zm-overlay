...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="debug doxygen gnome gsm kde networkmanager pulseaudio speex static-libs video"
...
RDEPEND="${CDEPEND}
		kde? ( net-voip/sflphone-client-kde )"

pkg_setup() {
	if ! use gnome && ! use kde ; then
		ewarn
		ewarn "No clients selected. Either select USE=gnome or USE=kde to get a graphicalclient."
		ewarn "See"
...
src_configure() {
	...
	econf --disable-dependency-tracking $(use_with debug) \
		$(use_with gsm) $(use_with networkmanager) $(use_with speex) $(use_enable static-libs static) $(use_enable doxygen) ${myconf}
	cd ..

	if use gnome; then
		#./hudson-sflphone-script.sh gnome
		cd gnome
...
src_install() {
	...
	dodoc test/sflphonedrc-sample || die "dodoc failed"
	cd ..
